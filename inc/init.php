...
function ms_global_search_style() {
	...
	if( file_exists( $styledir ) )
        ...
        $customstyleurl = MSGLOBALSEARCH_URL . '/customstyle.css';
        $customstyledir = MSGLOBALSEARCH_DIR . '/customstyle.css';

        if( file_exists( $customstyledir ) )
                wp_enqueue_style( 'ms_global_search_css_style', $customstyleurl );
}
