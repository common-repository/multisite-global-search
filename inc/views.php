...
function ms_global_search_build_views_drop( $trigger ) {
    ...
    $blogs = $wpdb->get_results( $wpdb->prepare( "SELECT blog_id, domain, path, public FROM {$wpdb->blogs} WHERE blog_id != %d AND site_id = %d AND public != '0' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ORDER BY registered DESC", $trigger, $wpdb->siteid ) );
...
function ms_global_search_build_views_add() {
    ...
    $blogs = $wpdb->get_results( $wpdb->prepare( "SELECT blog_id, domain, path, public FROM {$wpdb->blogs} WHERE site_id = %d AND public != '0' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ORDER BY registered DESC", $wpdb->siteid ) );
    ms_global_search_v_query( $blogs );
}
...
function ms_global_search_build_views_activate( $trigger ) {
    ...
    $blogs = $wpdb->get_results( $wpdb->prepare( "SELECT blog_id, domain, path, public FROM {$wpdb->blogs} WHERE ( blog_id = %d AND public != '0' AND archived = '0' AND mature = '0' AND spam = '0' ) OR ( site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ) ORDER BY registered DESC", $trigger, $wpdb->siteid ) );
...
function ms_global_search_build_views_unarchive( $trigger ) {
    ...
    $blogs = $wpdb->get_results( $wpdb->prepare( "SELECT blog_id, domain, path, public FROM {$wpdb->blogs} WHERE ( blog_id = %d AND public != '0' AND deleted = '0' AND mature = '0' AND spam = '0' ) OR ( site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ) ORDER BY registered DESC", $trigger, $wpdb->siteid ) );
    ms_global_search_v_query( $blogs );
}
...
function ms_global_search_v_query( $blogs ) {
    ...
    foreach ( $blogs as $blog ) {
        ...
        $posts_select_query .= " ( SELECT '{$blog->blog_id}' AS blog_id, '{$blog->domain}' AS domain, '{$blog->path}' AS path, '{$blog->public}' AS public,
            posts{$blog->blog_id}.ID,
        ...
        $postmeta_select_query .= " ( SELECT '{$blog->blog_id}' AS blog_id, '{$blog->domain}' AS domain, '{$blog->path}' AS path,  '{$blog->public}' AS public,
            postmeta{$blog->blog_id}.meta_id,
        ...
        $comments_select_query .= " ( SELECT '{$blog->blog_id}' AS blog_id, '{$blog->domain}' AS domain, '{$blog->path}' AS path,  '{$blog->public}' AS public,
            comments{$blog->blog_id}.comment_ID,
    ...
    if( $blogs != null ) {
		...
		if ( $wpdb->query( "CREATE OR REPLACE VIEW `{$wpdb->base_prefix}v_posts` AS $posts_select_query" ) === false ) {
		...
		if ( $wpdb->query( "CREATE OR REPLACE VIEW `{$wpdb->base_prefix}v_postmeta` AS $postmeta_select_query" ) === false ) {
			wp_die( __( 'Error creating search views in the database. <a href="plugins.php">Deactivate Multisite Global Search</a> and check you have create views privilege in your WordPress database.', 'ms-global-search' ).'<br />'. $wpdb->last_error );
		}
		
		if ( $wpdb->query( "CREATE OR REPLACE VIEW `{$wpdb->base_prefix}v_comments` AS $comments_select_query" ) === false ) {
			wp_die( __( 'Error creating search views in the database. <a href="plugins.php">Deactivate Multisite Global Search</a> and check you have create views privilege in your WordPress database.', 'ms-global-search' ).'<br />'. $wpdb->last_error );
		}
    } else {
...
function ms_global_search_drop_views() {
	...
	$wpdb->query( "DROP VIEW `{$wpdb->base_prefix}v_posts`" );
	$wpdb->query( "DROP VIEW `{$wpdb->base_prefix}v_postmeta`" );
	$wpdb->query( "DROP VIEW `{$wpdb->base_prefix}v_comments`" );
}
